...
LRESULT CALLBACK WndProc(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		if (points > 1)
		{
			MoveToEx(hdc, graph[points - 2].x, graph[points - 2].y, NULL);
			LineTo(hdc, graph[points - 1].x, graph[points - 1].y);
		}
		//Polyline(
	...
	case WM_LBUTTONUP:
		graph[points].x = LOWORD(lParam);
		graph[points].y = HIWORD(lParam);
		points++;
		InvalidateRect(hWnd, &actualDesktop, 0);
		break;
	case WM_DESTROY:
	{
		hPen = CreatePen(0, 0, RGB(0, 0, 0));
		PostQuitMessage(0);
		DeleteObject(hPen);
		/*DeleteObject(hBrush);*/
		break;
